...
const DEFEAULT_REQUEST_TIMEOUT = 10000; // 10 seconds
...
const sendRequestFailedMessageToTab = (tabId, error = null) => {
  if((error) && (error.status.code == 500))
  {
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1001, 'message':'An error occured. Please try again later.'}})
  }
  else if((error) && (error.status.code == 502))
  {
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1002, 'message':'Service unavailable for now. Please try again.'}})
  }
  else if((error) && (error.status.code == 503))
  {
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1003, 'message':'Service unavailable for now. Please try again.'}})
  }
  else if((error) && (error.status.code == 504))
  {
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1004, 'message':'Service unavailable for now. Please try again.'}})
  }
  else if((error) && (error.status.code == 505))
  {
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1005, 'message':'HTTP Version Not Supported. Please try again.'}})
  }
  else{
    chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: {'code': 1000, 'message':'Our servers are currently busy. Please try again later.'}})
  }
...
const fetchWithTimeout = (url, options, curation_init_request = false, timeout = DEFEAULT_REQUEST_TIMEOUT) => {
  ...
  fetchPromise.catch(error => {
    sendRequestFailedMessageToTab(options.tabId,error);
  })
...
window['Module'] = (function () {
  ...
  chrome.webNavigation.onDOMContentLoaded.addListener(function (info) {
    chrome.tabs.executeScript(null, { file: 'payload-before.js' }, _ => {
      console.log( 'executeScript' )
            ...
            if (e !== undefined) {
              console.log('execute payload-before.js')
            }
          })
  })
 ...
 async function handleBackendInit (request, reason, tabId, is_retry=false, metadata={}) {
      ...
      chrome.storage.local.get({'autorun': false}, storageObj => {
        if ((SUBSCRIBED_USER && storageObj.autorun) || reason === 'manual') {
          ...
          if (!localUrl) {
            ...
            globalRequest = fetchWithTimeout(apiurl, {credentials: 'include', tabId: tabId}, curation_init_request=true)
              .then(res => res.json())
              .then(res => {
    ...
    if (/theconversation.social/.test(url.href) && url.searchParams.has('link_id') && globalRequest === null) {
      ...
      if (localUrl) {
        ...
        chrome.tabs.getSelected(null, function(selectedTab) {
          ...
          if (selectedTab.id === sender.tab.id && globalRequest === null) {
            globalRequest = fetchWithTimeout(href, {
              method: request.data.method.toUpperCase(),
              credentials: 'include',
              tabId: sender.tab.id
              },
              curation_init_request=true)
              .then(res => res.json())
              .then(res => {
                if (res.errors && res.errors.length > 0)  {
                  processErrors(res.errors, sender && sender.tab.id)
  ...
  function openNoSubscriptionPage() {
    chrome.tabs.create({url: `chrome-extension://${chrome.runtime.id}/widget/no_subscription.html`}, function (tab) {
  ...
  function openNoTwitterAcctPage() {
    ...
    chrome.tabs.query({'url': `chrome-extension://${extensionId}/widget/no_twitter_acct.html`}, function(tabs) {
        if ( tabs.length > 0 ) {
            chrome.tabs.update(tabs[0].id,{'active':true});
        } else {
            chrome.tabs.create({'url':`chrome-extension://${extensionId}/widget/no_twitter_acct.html`});
        }
        noTwitterTab = tab;
    });
  }
